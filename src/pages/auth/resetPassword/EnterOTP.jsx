...
import { verifyResetPasswordOTP, requestResetPassword } from "../../../redux/actions/authActions";
...
const EnterOTP = () => {
    ...
    useEffect(() => {
        // Nếu không có email từ state, chuyển về trang forgot-password
        if (!email) {
        ...
        try {
            ...
            const handleVerifyResult = (result) => {
                if (result.success && result.token) {
                    // Chuyển đến trang nhập password mới với token
                    navigate(`/auth/reset-password/${result.token}`);
                } else {
            ...
            dispatch(verifyResetPasswordOTP({ email, otp: otpCode }, handleVerifyResult));
        } catch (error) {
    ...
    const handleResendOTP = async () => {
        ...
        try {
            ...
            // Gửi lại OTP
            dispatch(requestResetPassword({ email }, handleResendResult));
        } catch (error) {
